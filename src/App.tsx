...
export default function App() {
  ...
  const normalizeJapaneseText = (text = "") => {
    ...
    try {
      const tokens = tokenizer.tokenize(base);

      return tokens
        .map((token) => {
          return wanakana.toHiragana(token.reading || token.surface_form);
        })
        .join("")
        .toLowerCase();
    } catch (e) {
...
${template.support}`;
  ...
  const filteredTemplates = templates.filter((template) => {
    const keyword = normalizeJapaneseText(displayKeyword);
    ...
    const text = normalizeJapaneseText(
